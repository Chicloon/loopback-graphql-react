...
class ModelMultipleInput extends React.Component {
	...
	constructor (props) {
		...
		if (!this.props.preloadAll) filter.limit = this.perpage;
		this.list = this.store[this.modelName].list(filter);
	...
	componentDidUpdate() {
		if ((this.active || this.props.alwaysActive) && this.valueInput) {
	...
	@action toggleItem(e, item) {
		e.preventDefault();
		e.stopPropagation();
		console.log('toggleItem', item.id);
	...
	render () {
		...
		const items = this.list.list.map((item, i) => {
			const selected = this.props.isSelected && this.props.isSelected(item);
			...
			return <a key={item.id} href="#" className={'item' + (selected ? ' selected' : '')}
			          onClick={e => this.toggleItem(e, item)}>{value}</a>
		});

		const icon = <Icon className={"down-icon " + (this.active ? 'active' : '')} icon="down-open" onClick={e => this.toggle(e)} />;
		const more = !this.props.preloadAll && this.list.totalCount > this.perpage && this.list.totalCount > this.skip ? (
			<div className="more">
				{this.list.isLoading ?
					<Loader size={18} />
					:
		...
		return (
			<div className={'form-input ' + (this.props.className || '') + (this.props.inlineCreate ? ' inline-create' : '')}>
				{this.props.label && <label>{this.props.label}</label>}
				{this.props.selectedView || null}
				<input type="hidden" value={this.idValue || ''} />
				{
					!this.active && !this.props.alwaysActive ?
						<div className="drop-down" ref={node => this.node = node}>{input}{icon}</div>
						:
						<div className="drop-down active" ref={node => this.node = node}>
							...
							{/*{this.props.inlineCreate && <div><IconLink icon="plus" label={t('create')} /></div>}*/}
						</div>
				}
			</div>
		);
...
ModelMultipleInput.propTypes = {
	...
	preloadAll: PropTypes.bool,
	alwaysActive: PropTypes.bool,
	inlineCreate: PropTypes.bool,
	selectedView: PropTypes.object,
	label: PropTypes.string,
	placeholder: PropTypes.string,
};

ModelMultipleInput.defaultProps = {
	preloadAll: false,
	alwaysActive: false,
};

export default ModelMultipleInput;
